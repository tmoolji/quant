...
double BlackScholesPDE::boundary_left(double t, double x) const {
    //difference between spot and option strike discounted for T - t (put)
  return (option->K)*exp(-(option->r)*((option->T) - t)) - x;
}

double BlackScholesPDE::boundary_right(double t, double x) const {
  return 0.0; // this is for a put
}

double BlackScholesPDE::init_cond(double x) const {
  return option->pay_off->operator()(x); //initial condition for a vanilla put
}
